...
public class GuardianEndpoint {
    ...
    public static HttpResponse read(HttpPostRequest req,
        ...
        try {
            RequestModel<Guardian> requestModel = RequestModel.fromJson(Guardian.class, new String((byte[]) req.getContent().getData()));
            return res.setContent(CommonService.read(Guardian.class, requestModel).parse());
        } catch (Exception e) {
            logger.atError().log(e.getMessage());
            return res.setContent(new Response<Guardian>().setError(ResponseError.JSON_PARSE_ERROR).parse());
        }
    }
    ...
    public static HttpResponse create(HttpPostRequest req,
                                      HttpResponse res) {
        var auth = AuthenticationService.authenticate(req, Set.of(Role.ADMIN), Set.of(AccessLevel.SUPERUSER, AccessLevel.GUARDIAN));
        if (auth != null) return auth;

        try {
            RequestModel<Guardian> requestModel = RequestModel.fromJson(Guardian.class, new String((byte[]) req.getContent().getData()));
            return res.setContent(CommonService.create(Guardian.class, requestModel).parse());
        } catch (Exception e) {
            logger.atError().log(e.getMessage());
            return res.setContent(new Response<Guardian>().setError(ResponseError.JSON_PARSE_ERROR).parse());
        }
    ...
    public static HttpResponse update(HttpPostRequest req,
        ...
        try {
            RequestModel<Guardian> requestModel = RequestModel.fromJson(Guardian.class, new String((byte[]) req.getContent().getData()));
            return res.setContent(CommonService.update(Guardian.class, requestModel).parse());
        } catch (Exception e) {
            logger.atError().log(e.getMessage());
            return res.setContent(new Response<Guardian>().setError(ResponseError.JSON_PARSE_ERROR).parse());
        }
    ...
    public static HttpResponse delete(HttpPostRequest req,
        ...
        try {
            RequestModel<Guardian> requestModel = RequestModel.fromJson(Guardian.class, new String((byte[]) req.getContent().getData()));
            return res.setContent(CommonService.delete(Guardian.class, requestModel).parse());
        } catch (Exception e) {
            logger.atError().log(e.getMessage());
            return res.setContent(new Response<Guardian>().setError(ResponseError.JSON_PARSE_ERROR).parse());
        }
